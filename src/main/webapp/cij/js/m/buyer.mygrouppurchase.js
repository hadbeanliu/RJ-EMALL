...
//			order: [["aa", "desc"]],
			...
			columns: [{
				...
				data: "id",
				name: "<th class='small-table-cell'>商品名称</th>",
				orderable: true,
				searchable: true,
				display: function (td, value, vo, rowIndex, colIndex) {
					var goods = vo.goods;
				...
				data: "id",
				name: "<th class='min_w90'>单价*数量</th>",
				searchable: true,
				display: function (td, value, vo, rowIndex, colIndex) {
					var goods = vo.goods;
					var htm = [];
					htm.push('<div class="show-for-small left">单价*数量：</div>');
					for ( var i = 0; i < goods.length; i++) {
						var gs = goods[i];
						htm.push('<div class="'+ (i !== goods.length-1 ? 'min_h95 ' : '') +'">'+ gs.purchasePrice +'*'+gs.purchaseQuantity+'</div>');
					}
				...
				data: "price",
				name: "<th class='min_w110'>实付款（元）</th>",
				orderable: false,
				searchable: true,
				display: function (td, value, vo, rowIndex, colIndex) {
