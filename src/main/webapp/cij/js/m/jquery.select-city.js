(function($, E){
  ...
  //创建InfoWindow
	...
	selectareas.initArea = function(callback){
		$.ajax({
			type:"POST",
			url: Cfg.contextPath + "/admin/areas/areasselect",
			dataType : "json",
	...
	selectareas.init=function($elem){
		var areaData;
		var $lt=$elem;
		...
		if(! areaData){
			selectareas.initArea(function(data){
				areaData=data;
				$lt.find('.area-select').each(function(){
					var $op=$(this);
...
//							setPlace();
	...
	$(function() {
		...
		$('.button').click(function(){
			E.selectareas.acccordToCode($('#abc'),'120101');
		});
		$('.getcode').click(function(){
			var code=E.selectareas.getSelectedAreaCode($('#abc'));
			if(code!=undefined)
			{
				alert(code);
			}
